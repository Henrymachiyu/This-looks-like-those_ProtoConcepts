...
def soft_clamp(x, val=1.0, upper=True): 
    # a numpy version of soft-clamp
    if upper: 
        return torch.min(x, val + x - x)
    else: 
        return torch.max(x, val + x - x)
...
def find_k_nearest_patches_to_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    cap_width = prototype_network_parallel.module.cap_width_l2
    cap_factor = prototype_network_parallel.module.cap_activation(cap_width)
