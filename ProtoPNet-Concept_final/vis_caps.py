...
from helpers import makedir, find_high_activation_crop, upscale_rf
...
def update_prototypes_on_batch(search_batch_input,
    ...
    with torch.no_grad():
        ...
        if ltwo: 
            proto_act_torch, cap_factor_torch = prototype_network_parallel(search_batch,get_act=False, vis_cap=True)
            ...
            cap_factor_torch = cap_factor_torch.unsqueeze(2).unsqueeze(3)
            max_act_l2 = 1.0
    ...
    for i in range(len(patches_to_vis)): 
        ...
        upsampled_act_img_j = upscale_rf(protoL_rf_info, torch.tensor(proto_act_img_j), original_img_size)
        ...
        # remove the repeating vis by activiated adjacent patches 
